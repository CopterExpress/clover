...
Среда симуляции Клевера позволяет пользователям запускать и отлаживать свой код в симуляторе, используя большинство функций, доступных на реальном дроне. Симулятор использует [PX4 SITL mode](sitl.md) и тот же ROS-код, что и настоящий дрон. Большинство железа также симулируется.

## Особенности

Устанавливаемая пользователем среда включает в себя:

* высококачественную модель Клевер 4;
* плагины Gazebo для железа Клевера (например, светодиодная лента);
...
* Веб-сервер (Monkey) для плагинов Клевера, требующих подключение к интернету;
