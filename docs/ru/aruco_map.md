...
По аналогии с [навигацией по отдельным маркерам](aruco_marker#навигация-по-маркерам) при настройке карты маркеров дрон сможет лететь в координаты относительно отдельного маркера, используя фрейм aruco_ID с соответствующим ID маркера. Эта функция будет работать даже в случае, если дрон не видит в данный момент нужный маркер.
