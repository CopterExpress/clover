...
В данном коде для вас особенно важны основные строки настройки ленты:

+ LED_COUNT - Количество светодиодов в ленте
+ LED_PIN - GPIO пин, к которому вы будете подсоединять светодиодную ленту
...
Для подключения библиотеки и её корректной работы требуется подключить следующие модули: `rpi_ws281x` - для работы ленты и `time` – для управления задержками.
