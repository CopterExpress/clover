...
- Перепрошить полетный контроллер модифицированной прошивкой. Скачать её можно [здесь[(firmware.md).

- Инструкция по прошивке и настройке полетного контроллера — в [статье](setup.md) в разделе «Обновление прошивки Pixhawk». Следующий раздел выполнять не нужно, если коптер уже был откалиброван.
...
- Настройка описана в [статье](setup.md) в разделе «Настройка режимных каналов».
...
В [этой](simple_offboard.md) статье описана работа с модулем Simple Offboard, который создан для простого программирования дрона. В ней даны описания основных функций, а также примеры кода.
...
Необходимо тщательно подготовить дрон, пульт и программу. Произвести selfcheck. Убедиться, что дрон летает в ручно режиме.
